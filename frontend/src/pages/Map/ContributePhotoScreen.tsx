...
export default function ContributePhotoScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/${reportId}/contribute`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      if (!res.ok) throw new Error((await res.text()) || `HTTP ${res.status}`);
      toast({ title: "✅ Contribution submitted! +15 points", status: "success" });
  ...
  return (
    <Container maxW="sm" centerContent py={10}>
      <VStack spacing={6} w="100%">
        <Heading size="lg">📸 Upload Your Cleanup Photo</Heading>
        <Text color="gray.500" textAlign="center">
          Show the area you helped clean up
        </Text>
        <Box w="100%" maxW="300px" h="250px" borderWidth="2px" borderStyle="dashed" borderRadius="lg" display="flex" alignItems="center" justifyContent="center" bg={photo ? "transparent" : "gray.50"} overflow="hidden">
          {photo ? (
            <img src={photo} alt="Cleanup" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
            <Text color="gray.400">Tap to take a photo</Text>
          )}
        </Box>
        <Button as="label" htmlFor="contrib-camera" colorScheme="blue" w="100%">
          {photo ? "📷 Retake" : "📷 Take Photo"}
          <input id="contrib-camera" type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={handleFileChange} />
        </Button>
